...
def show_admin():
    ...
    with tab_users:
        ...
        if df_users.empty:
            ...
            for _, row in df_users.iterrows():
                ...
                badge = "U0001f7e2" if active else "U0001f534"
        ...
        if df_logs.empty:
            ...
            for _, row in filtered.sort_values("timestamp", ascending=False).iterrows():
                with st.container(border=True):
                    ...
                    h1.markdown(f"**{row['timestamp']}** | 👤 {row['user_name']} \n📍 수신싳: {row['client']} | 🏗️ 공사명: {row['project']} \n💰 견적금액: **{amt_str}**")
                    status = row["contract_done"]
                    h2.markdown(f"{'U0001f7e2' if status == '계약완료' else 'U0001f534'} **{status}**")
                    if status == "미계약":
    ...
    with tab_stats:
        ...
        if df_logs.empty:
            ...
            col1.metric("전체 견적 건수", f"{len(df_logs)}건")
...
def show_main():
    ...
    with col_l:
        ...
        client = st.text_input("수신싳", placeholder="입력하세요 (예: OO설계사무소)")
        ...
        with t_w:
            ...
            if st.button("➕ 폐기물 항목 추가"):
                ...
                st.session_state["q_items"].append({"품명": w_type, "규격": f"L={m_dist if dist_mode == '60km 초과' else dist_mode}", "수량": w_qty, "단위": "ton", "단가": unit, "금액": unit * w_qty})
        with t_a:
            ...
            if st.button("➕ 골재 항목 추가"):
                st.session_state["q_items"].append({"품명": "순환골재", "규격": a_spec, "수량": a_qty, "단위": "m³", "단가": a_price, "금액": a_qty * a_price})
